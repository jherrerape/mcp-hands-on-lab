...
// Importante: el SDK registra el nombre en minúsculas (primera letra del método), aunque en C#
// el método se llame "Add". Verifica siempre el nombre exacto con ListToolsAsync() si tienes dudas.
...
| `McpProtocolException: Request failed (remote): Unknown tool: 'Add'` (C#) | El nombre de la tool distingue mayúsculas/minúsculas. El SDK registra el método `Add` como `"add"` (minúscula). Usa siempre el nombre exacto que ves impreso al listar tools con `ListToolsAsync()` |
